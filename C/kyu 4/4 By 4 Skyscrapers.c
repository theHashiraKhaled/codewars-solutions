...
//Don't bully me I know my code is really ugly );
struct cell
{
...
void edge_clue_initialization(struct board *board, int *clues)
{
    for (int i = 0; i < board->cell_count; i++)
    {
      // Adjacent cell resolved to BOARD_SIZE
       if (clues[i] == 1) 
            for (int p = 1; p < BOARD_SIZE; p++)
                remove_possibility(&(board->cells[clue_index_on_board(i)]), p);
      // row/colmn resolved to {1..N}
        else if (clues[i] == BOARD_SIZE) 
        {
            if (is_clue_on_row(i))
            {
                if (i >= 3 * BOARD_SIZE)
